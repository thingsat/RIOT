...
void *_csp_server_thread(void *arg)
{
    ...
    while (1) {
        ...
        if (packet->id.dport == _server_port) {
...
static void _start_server(const char *port_str)
{
    ...
    if (port == 0 || port > CSP_PORT_MAX_BIND) {
        printf("ERROR: invalid port specified, Bind port < %d\n", CSP_PORT_MAX_BIND);
        return;
    }
...
static int _set(char *cmd_name, char *key, char *value)
{
    (void)cmd_name;
...
static int _csp_handler(int argc, char **argv)
{
    ...
    //     return _get(argv[0], argv[2], argv[3]);
    // }
    if (strcmp(argv[1], "set") == 0) {
        ...
        return _set(argv[0], argv[2], argv[3]);
    }
